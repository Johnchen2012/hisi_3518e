...
{
    PAYLOAD_TYPE_E enPayLoad[3]= {PT_H264, PT_H264,PT_H264};//视频编码格式
    ...
    VENC_CHN VencChn;
    SAMPLE_RC_E enRcMode= SAMPLE_RC_CBR; //恒定比特率的方式进行编码，h.264的编码方式很多种
    ...
    ******************************************/
    memset(&stVbConf,0,sizeof(VB_CONF_S));//清VB
    
	SAMPLE_COMM_VI_GetSizeBySensor(&enSize[0]);//由于硬件觉得采集的码流分辨率
    if (PIC_HD1080 == enSize[0])
    {
        enSize[1] = PIC_VGA;
		s32ChnNum = 2;
    }
    else if (PIC_HD720 == enSize[0]) //应该是进到这里来了分辨率为1080*720
    {
        enSize[1] = PIC_VGA;			
		enSize[2] = PIC_QVGA;
		s32ChnNum = 3;         //采集到的视频码流分成了分辨率大小不一样的三路码流
    }
    else
    {
...
/***  新增的代码，只做rtp方式输出，所以只输出一路码流通道 ***/
	//s32ChnNum = 1;//只是单纯的做网络传输，所以只要一路就ok

	
	printf("s32ChnNum = %d\n",s32ChnNum);

    stVbConf.u32MaxPoolCnt = 128; //用户设置vb_pool的最大值
    ...
    //一帧图片所占内存大小
	    u32BlkSize = SAMPLE_COMM_SYS_CalcPicVbBlkSize(gs_enNorm,\
	                enSize[0], SAMPLE_PIXEL_FORMAT, SAMPLE_SYS_ALIGN_WIDTH);//扫描方式，分辨率大小，像素格式，对齐排布(向上取整)
	    stVbConf.astCommPool[0].u32BlkSize = u32BlkSize; //vb_block大小
	    stVbConf.astCommPool[0].u32BlkCnt = g_u32BlkCnt;//vb_block个数
	}
	if(s32ChnNum >= 2)
    {
	    u32BlkSize = SAMPLE_COMM_SYS_CalcPicVbBlkSize(gs_enNorm,\
    ...
    ******************************************/
    stViConfig.enViMode   = SENSOR_TYPE;//sensor类型 ,传入的sensor类型，
    stViConfig.enRotate   = ROTATE_NONE;//翻转
    stViConfig.enNorm     = VIDEO_ENCODING_MODE_AUTO;//制式
...
int main(int argc, char *argv[])
{
    ...
    signal(SIGINT, SAMPLE_VENC_HandleSig);//signal信号
